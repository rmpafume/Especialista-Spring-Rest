...
public class RestauranteProdutoFotoController {
	...
	@Autowired
	private FotoProdutoModelAssembler fotoModelAssembler;
	...
	public FotoProdutoModel atualizar(@PathVariable Long restauranteId, 
		...
		return fotoModelAssembler.toModel(fotoSalva);
	}
	...
	public ResponseEntity<?> servir(@PathVariable Long restauranteId, @PathVariable Long produtoId, 
		...
		try {
			...
			if (fotoRecuperada.temUrl()) { //implementação amazon s3
				return ResponseEntity
						.status(HttpStatus.FOUND)
						.header(HttpHeaders.LOCATION, fotoRecuperada.getUrl())
						.build();
			}else { // implementação local
